...
class DocciDataset(Dataset):
    def __init__(self, split='train', max_samples=None):
        self.data = load_dataset("google/docci", 
                                 trust_remote_code=True, 
                                 cache_dir="data2/",
                                 storage_options={'client_kwargs': {'timeout': aiohttp.ClientTimeout(total=3600)}})[split]
...
if USE_WANDB:
    import wandb
    wandb.init(
        project="moondream-ft-sgSMU-docci-dataset",
        config={
            "EPOCHS": EPOCHS,
            "BATCH_SIZE": BATCH_SIZE,
...
# TODO: Save the model 
# Save the model
moondream.save_pretrained("checkpoints/moondream-ft-sgSMU-docci-dataset")
