...
pynvml.nvmlInit()
device = pynvml.nvmlDeviceGetHandleByIndex(0)  # Change to appropriate GPU index if needed
...
# Function to get system RAM usage
def get_system_memory():
    memory = psutil.virtual_memory()
    return memory.used / (1024 ** 2)  # in MB
...
# Freeze base model weights and enable LoRA
if use_lora:
...
# Using standard PyTorch Adam optimizer instead of bitsandbytes
...
for epoch in range(EPOCHS):
    ...
    for batch_idx, batch in enumerate(tqdm(dataloaders["train"], desc=f"Epoch {epoch + 1}/{EPOCHS}")):
        ...
        print(f"GPU Memory: {gpu_memory:.2f} MB | System RAM: {system_memory:.2f} MB")
